...
class Head_Block(Module):
    def __init__(self, num_filter, multiplier, head_pooling=False, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)):
        ...
        # RGB~[~C~O~L~E~P3个~@~Z~A~S~Hin_channels~I
...
class Embedding_Block(Module):
    def __init__(self, input_channels, last_channels, emb_size, bias=False):
        ...
        # depthwise(1),~S~E为224*224~W~L~O~Fkernel_size~T为(14, 14)
        self.conv1 = Conv2d(in_channels=last_channels, out_channels=last_channels, kernel_size=(14, 14), stride=(1, 1),
...
class VarGFaceNet(Module):
    def __init__(self):
        ...
        self.emb = Embedding_Block(input_channels=int(filter_list[3] * multiplier), last_channels=last_channels,
                                   emb_size=emb_size, bias=False)  # ~P代| ~A~Z~Dinput_channels缺~Q*multiplier~L~W| ~U~P~L
        # initialization
        '''
        for m in self.modules():  # ~@~_~TMobileNetV3~Z~D~H~]~K~L~V~V~U
